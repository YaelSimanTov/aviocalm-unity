...
public class NpcMovement : MonoBehaviour
{
    ...
    public void MoveAiToTarget(Transform target)
    {
        ...
        {
            // Tagged as [Dev] because this is an internal warning for the developer
            Debug.LogWarning("[Dev] target is Null");
        }
    }
    ...
    public void MoveNpcToButton(Transform Button)
    {
        if (Button != null)
        {
                ...
                (!NavMeshAgentAi.hasPath || NavMeshAgentAi.velocity.sqrMagnitude == 0f))
            {
                ...
                // Clinical log: Marks the exact moment the safety figure (attendant) arrives to help the patient
                Debug.Log("[System Event] Flight attendant arrived at the patient's seat to provide assistance.");
    ...
    public void MoveNpcToExit(Transform Exit)
    {
        if (Exit != null)
        {
            // Tagged as [Dev] to avoid cluttering the clinical timeline
            Debug.Log("[Dev] Moving NPC to exit");
